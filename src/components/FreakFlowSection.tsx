...
const socialPosts = [
  { platform: "TikTok", handle: "@freakshow.pb", caption: "That haptic feedback hit different 🔥 #pickleballfreakshow #gen3", likes: "234K", comments: "1.2K", image: athleteUrban, product: "Gen 3 Haptic Pro", price: "$289", tall: true },
  { platform: "Instagram", handle: "@maya.strikes", caption: "Underground sessions with the crew. New paddle, new vibes.", likes: "89K", comments: "847", image: athleteUrban, product: "Renegade Tee", price: "$65", tall: false },
  { platform: "YouTube", handle: "FreakshowTV", caption: "GEN 3 HAPTIC PRO — FULL REVIEW | Is this the future of pickleball?", likes: "1.4M", comments: "23K", image: athleteUrban, product: "Gen 3 Haptic Pro", price: "$289", tall: false },
  { platform: "TikTok", handle: "@dink.master", caption: "The Neural-Grip sensors are INSANE. My swing data is 📈", likes: "567K", comments: "3.4K", image: athleteUrban, product: "Cyber-Mesh Shorts", price: "$78", tall: true },
  { platform: "Instagram", handle: "@freakshow.pb", caption: "Drop 003 incoming. Are you ready? 👁️", likes: "156K", comments: "2.1K", image: athleteUrban, product: "Freakshow Tech-Hat", price: "$48", tall: false },
];
...
const FreakFlowSection = () => {
  ...
  const handleShopClick = (productName: string) => {
    ...
    if (mapping) {
      navigate(`/${mapping.type}/${mapping.slug}`);
    }
  };
        ...
        <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 space-y-4">
          {socialPosts.map((post, index) => (
            <div
              key={index}
              data-index={index}
              className={`break-inside-avoid group relative border border-border bg-card rounded overflow-hidden transition-all duration-700 ${visibleCards.has(index) ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
            ...
            >
              ...
              <div className="p-4">
                ...
                <div
                  className="flex items-center justify-between p-3 bg-surface rounded border border-border group-hover:neon-border-lime transition-all duration-300 cursor-pointer"
                  onClick={() => handleShopClick(post.product)}
                >
                  <div>
                  ...
                  <Button variant="neonLime" size="sm" onClick={(e) => { e.stopPropagation(); handleShopClick(post.product); }}>
